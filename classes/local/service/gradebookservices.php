...
class gradebookservices extends service_base {
    ...
    public static function complete_remapping($courseid) {
        global $DB;
        $params = array('courseid' => $courseid);
        $sql = "SELECT i.id as gradeitemid, g.id as gbsid
                  FROM {grade_items} i
                  JOIN {ltiservice_gradebookservices} g on i.id=g.gradeitemid
                 WHERE g.courseid = :courseid and g.courseid <> i.courseid";
        $toremap = $DB->get_records_sql($sql, $params);
        $sql_newid="SELECT id from {grade_items} where itemnumber = :origid and courseid = :courseid";
        $sql_update_gbs="UPDATE {ltiservice_gradebookservices} SET gradeitemid = :gradeitemid where id = :id";
        $sql_update_gi="UPDATE {grade_items} SET itemnumber = :itemnumber where id = :id";
        $sql_delete_orphan_gbs="DELETE FROM {ltiservice_gradebookservices} where id = :id";
        foreach ($toremap as $r) {
            $params = array('courseid' => $courseid, 'origid' => $r->gradeitemid);
            $newitems = $DB->get_records_sql($sql_newid, $params);
            if (count($newitems) > 0) {
                $newitem = reset($newitems);
                $DB->execute($sql_update_gi, array('id'=>$newitem->id, 'itemnumber'=>$newitem->id));
                $DB->execute($sql_update_gbs, array('gradeitemid'=>$newitem->id, 'id'=>$r->gbsid));
            } else {
                $params = array('id' => $r->gbsid);
                $DB->execute($sql_delete_orphan_gbs, $params);
            }
        }
    }
